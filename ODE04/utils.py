...
def read_data(args, A_lst):
    ...
    # PEMSD7M == shape: (12672, 228, 1)
    # PEMSD7L == shape: (12672, 1026, 1)
    #num_node = data.shape[1] #num_node=307
    ...
    # dist_matrix = np.load('08_all_pre_adj_V6.npy')
    # #写个循环，每次输入一个邻接矩阵
    # # mean_v = np.mean(dist_matrix)                 #原先没负号
    # # std = np.std(dist_matrix)
    # # dist_matrix = (dist_matrix - mean_v) / std
    # sigma = args.sigma1
    ...
    # use continuous spatial matrix 使用连续空间矩阵
    # if not os.path.exists(f'data/{filename}_spatial_distance.npy'):
    #     with open(filepath + file[1], 'r') as fp: #file[1]:PEMS08/distance.csv'
    ...
    # dtw_avg_degree = np.sum(dtw_matrix > 0)/2/num_node
    # sp_avg_degree = np.sum(sp_matrix > 0)/2/num_node

    #return data, mean_value, std_value, dtw_matrix, sp_matrix, dtw_avg_degree, sp_avg_degree
