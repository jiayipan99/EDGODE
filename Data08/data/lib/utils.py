...
def compute_val_loss(net, val_loader, loss_function, A_val, device, epoch):
	net.eval()
	temp =0
	with torch.no_grad():
		tmp = []
		for index, (val_r, val_t) in enumerate(val_loader):
			...
			output, _, _ = net(val_r, A_val[temp])
...
def predict(net, test_loader, A_test, device):
	'''
	predict
	Parameters
	----------
	...
	with torch.no_grad():
		...
		for index, (test_r, test_t) in enumerate(test_loader):
			...
			output, _, _ = net(test_r, A_test[temp])
		...
		for index, (test_r, test_t) in enumerate(test_loader):
			...
			test_r = test_r.to(device)
			test_t = test_t.to(device)
			_, spatial_at, temporal_at = net(test_r, A_test[temp])
