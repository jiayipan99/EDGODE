...
def prepare_dataloader(A, opt):
    ...
    print('[Info] Loading 08_all(train,val,test) data...')
...
def train_hawkes(model, training_data, optimizer, scheduler, pred_loss_func, opt):
    ...
    for epoch_i in range(opt.max_epoch):
        ...
        lambda_list = lambda_list.reshape(28900, 864)
        ...
        for t in range(864):
            ...
            L_list = []
            for j in range(0, len(L), 170):
                b = L[j:j + 170]
                L_list.append(b)
            lambda_mat.append(L_list)
        #np.save(('./04_all_lambda-V1.npy'), lambda_mat)
