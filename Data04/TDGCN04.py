...
parser.add_argument('--seed', type=int, default=34250, help='default=31150', required=False)
...
if __name__ == "__main__":
    ...
    if not load_matrix:
        ...
        np.save('./04adj_V5.npy', A_lst)
        print("Saved.")
        A_lst = np.array(A_lst)
        # exit(0)
    else:
        print("Loading Adjacency matrix...  ")
        A_lst = np.load('04_all(shuffle=Ture,809).npy')

    A = A_lst
    A = np.load('04adj_V5.npy')
    A = A[0:809]
    A[np.isnan(A)] = 0.
    A[np.isinf(A)] = 0.
    ...
    np.save('./04pre_adj_V5.npy', A_pre)
...
"""
    ...
    A_pre = np.load('04pre_adj_V2.npy')

    "04数据集，train/val/test 划分（485/162/162）"
    A_all=np.load('04adj_V1.npy')
    A_all[np.isnan(A_all)] = 0.
    A_all[np.isinf(A_all)] = 0.
    A_all[np.isinf(A_all)]=0
    A_all[np.isnan(A_all)]=0
    A_all = torch.tensor(A_all*KMD).type(torch.float32).to(device)
    A_train = A_all[:486]
    A_val = A_all[486:648]
    A_test = A_all[648:]
    ...
    for epoch in range(1, epochs + 1):
        ...
        if validation_loss_lst[arg] < 925.0 and learning_rate == 0.001:#04:925,#08:710
