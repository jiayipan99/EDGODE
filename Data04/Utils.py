...
from TDGCN.Data08.transformer.Models import get_non_pad_mask
...
def compute_event(event, non_pad_mask):
    ...
    # add 1e-9 in case some events have 0 likelihood 添加1e-9，以防某些事件的可能性为0
    event += math.pow(10, -9)
...
def log_likelihood(model, data, time, types):
    ...
    #print("all_lambda.size:", len(all_lambda))
    type_lambda = torch.sum(all_lambda * type_mask, dim=2)
    #print("type_lambda:", type_lambda)

    # event log-likelihood
    event_ll = compute_event(type_lambda, non_pad_mask)
    event_ll = torch.sum(event_ll, dim=-1)

    #lambda_list = type_lambda.tolist()[0]
    lambda_list = type_lambda.cpu().detach().numpy()
    #print("lamdba_list:",lambda_list)
    ...
    return event_ll, non_event_ll, lambda_list
...
def type_loss(prediction, types, loss_func):
    ...
    # 将基于[1,2,3]的类型转换为[0,1,2];同时将填充事件转换为-1进入翻译页面
    truth = types[:, 1:] - 1
...
def time_loss(prediction, event_time):
    ...
    # event time gap prediction 事件时间差预测
